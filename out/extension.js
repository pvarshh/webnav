...
function activate(context) {
    ...
    async function getAgentPrompt(workspaceRoot, roleFile) {
        try {
...
Remember: You are executing a task for the user based on the chat history. Provide ONLY the final executable output for your role.`;
    ...
    async function parseAndWriteFiles(agentOutput, workspaceRoot, stream) {
    ...
    async function runAgent(roleFile, state, workspaceRoot, stream, token) {
        ...
        if (!model)
            throw new Error("Copilot model not found. Ensure GitHub Copilot is active.");
